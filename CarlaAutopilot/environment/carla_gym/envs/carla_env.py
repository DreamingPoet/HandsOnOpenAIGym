...
from environment.carla_gym.envs.carla_main.world import World


# Set this to the path to your Carla binary
# SERVER_BINARY = os.environ.get(
#     "CARLA_SERVER", os.path.expanduser("E:/CloudWorkSpace/carla/WindowsNoEditor/CarlaUE4.exe"))
...
ENV_CONFIG = {
    ...
    # Exclude high-level planner inputs & goal info from the observations
...
DISCRETE_ACTIONS = {
    ...
    3: [1.0, 0.0],    # Forward
    4: [1.0, -0.5],   # Bear Left & accelerate
    5: [1.0, 0.5],    # Bear Right & accelerate
}
...
# To keep track of all the Carla processes we launch to make the cleanup easier
...
class Transform():
    def __init__(self, location, orientation, rotation):
        self.location : Vector3D = location
        self.orientation : Vector3D = orientation
        self.rotation : Rotation3D = rotation

    #可选构造函数
    @classmethod
    def zero(cls):
        return cls(Vector3D.zero(), Vector3D.zero(), Rotation3D.zero())


class PlayerMeasurements():
    def __init__(self):
        self.transform: Transform = Transform.zero()
        self.forward_speed: float = 0.0
        self.forward_distance: float = 0.0
        self.collision_vehicles: float = 0.0
        self.collision_pedestrians: float = 0.0
        self.collision_other: float = 0.0
        self.intersection_otherlane: float = 0.0
        self.intersection_offroad: float = 0.0
...
class CarlaEnv(gym.Env):
    def __init__(self, config=ENV_CONFIG):
        ...
        self.observation_space = Box(-128.0, 128.0, shape=(2,), dtype=np.float32) # forward_speed, dist to goal
    ...
    def clear_server_state(self):
        print("clear_server_state")
        # 删除原来的 world 包含车辆和传感器等
    ...
    def reset_env(self):
        ...
        # 新建世界
        self.world = World(self.sim_world, args)

        print("Starting new episode...")

        py_measurements = self._read_observation()
        self.prev_measurement = py_measurements
        return self.encode_obs(py_measurements)

    def encode_obs(self, py_measurements):

        obs = [py_measurements["forward_speed"],
    ...
    def step(self, action):
        try:
            ...
            self.clear_server_state()
            return (self.last_obs, 0.0, True, True, {})
    ...
    def step_env(self, action):
        ...
        # 发送控制
        self.world.apply_control(throttle, steer, brake )
        ...
        # 计算结束条件
        done = (self.num_steps > MAX_STEP or
        ...
        return (
            self.encode_obs(py_measurements), reward, done, done,
            py_measurements)
    ...
    def _read_observation(self):
        ...
        forward_distance = self.world.player_start_loction.distance(t.location)

        col_res = self.world.collision_sensor.get_collision(self.world.frame)
        lane_check = self.world.lane_invasion_sensor.lane_check(self.world.frame)
        measurements = Measurements()
        pm = PlayerMeasurements()
        pm.transform.location = t.location
        pm.transform.orientation = t.get_forward_vector()
        pm.transform.rotation = t.rotation
        pm.forward_speed = speed
        pm.forward_distance = forward_distance
        pm.collision_vehicles = col_res[0]
        pm.collision_pedestrians = col_res[1]
        pm.collision_other = col_res[2]
        pm.intersection_otherlane = lane_check[0]
        pm.intersection_offroad = lane_check[1]

        measurements.player_measurements = pm
        ...
        py_measurements = {
            ...
            "forward_distance": cur.forward_distance,
    ...
    def calculate_reward(self, current_measurement):
        ...
        cur_dist = current_measurement["forward_distance"]

        prev_dist = self.prev_measurement["forward_distance"]
        ...
        # Change in distance (km/hr)
        reward += 0.1 * \
            (cur_dist -
             prev_dist)
        # print("distance reward = " + str( 0.1*(cur_dist - prev_dist) ) )


        # Change in speed (km/hr)
        reward += 0.05 * \
            (current_measurement["forward_speed"] -
...
def print_measurements(measurements : Measurements):
    ...
    for _ in range(5):
        ...
        while not done:
            ...
            if ENV_CONFIG["discrete_actions"]:
                ...
                # Full throttle, zero steering angle
