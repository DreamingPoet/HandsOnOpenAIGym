...
# With the intent to make the results reproducible
...
class DeepActorCriticAgent(mp.Process):
    def __init__(self, id, env_name, agent_params, env_params):
        ...
        # Agent's personal best mean episode reward
        self.best_mean_reward = - float("inf")
        self.best_reward = - float("inf")
        # Whether or not the params have been saved along with the model to model_dir
        self.saved_params = False
        # Assumption by default unless env.action_space is Discrete
    ...
    def multi_variate_gaussian_policy(self, obs):
        ...
        self.action_distribution = MultivariateNormal(
            self.mu, torch.eye(self.action_shape) * self.sigma, validate_args=True)
    ...
    def calculate_n_step_return(self, n_step_rewards, final_state, done, gamma):
        ...
        with torch.no_grad():
            ...
            # Reverse order; From r_tpn to r_t
    ...
    def calculate_loss(self, trajectory, td_targets):
        ...
        for td_target, critic_prediction, log_p_a in zip(td_targets, v_s_batch, log_prob_a_batch):
            ...
            # td_err is an unbiased estimated of Advantage
            actor_losses.append(- log_p_a * td_err)
            critic_losses.append(F.smooth_l1_loss(
                critic_prediction, td_target))
        ...
        writer.add_scalar(self.actor_name + "/critic_loss",
                          critic_loss, self.global_step_num)
        writer.add_scalar(self.actor_name + "/actor_loss",
                          actor_loss, self.global_step_num)

        return actor_loss, critic_loss

    def learn(self, n_th_observation, done):
        if self.params["clip_rewards"]:
            # Clip rewards to -1 or 0 or +1
            self.rewards = np.sign(self.rewards).tolist()
        td_targets = self.calculate_n_step_return(
            self.rewards, n_th_observation, done, self.gamma)
        actor_loss, critic_loss = self.calculate_loss(
            self.trajectory, td_targets)
    ...
    def save(self):
        model_file_name = self.params["model_dir"] + \
            "A2C_" + self.env_name + ".ptm"
    ...
    def load(self):
        ...
        agent_state = torch.load(
            model_file_name, map_location=lambda storage, loc: storage)
    ...
    def run(self):
        ...
        for game in Atari.get_games_list():
            if game.replace("_", "") in args.env.lower():
                atari_env = True
        # Use the Atari wrappers (like we did in Chapter 6) if it's an Atari env
        if atari_env:
            print("make atari_env")
        ...
        if len(self.state_shape) == 3:  # Screen image is the input to the agent
            ...
            if self.continuous_action_space:
                ...
                self.actor = ShallowActor(
                    self.state_shape, self.action_shape, device).to(device)
            else:  # Discrete action space
                self.actor = ShallowDiscreteActor(
                    self.state_shape, self.action_shape, device).to(device)
            self.critic = ShallowCritic(
                self.state_shape, self.critic_shape, device).to(device)
        ...
        # To keep track of the num of ep with higher perf to save model
            ...
            while not done:
                ...
                if not args.test and (step_num >= self.params["learning_step_thresh"] or done):
                    ...
                    if done:
                        ...
                        print("mean_episode_rewards = " + str(mean_episode_rewards))


                        print("prev_checkpoint_mean_ep_rew = " + str(prev_checkpoint_mean_ep_rew))
                        # 平均分如果大于 之前检查点的 最高的平均分
                        if  mean_episode_rewards > prev_checkpoint_mean_ep_rew:
                            num_improved_episodes_before_checkpoint += 1

                        print("num_improved_episodes_before_checkpoint = " + str(num_improved_episodes_before_checkpoint))
                        # if num_improved_episodes_before_checkpoint >= self.params["save_freq_when_perf_improves"]:

                        # 如果当前得分 大于 当前平均分
                        if ep_reward >= mean_episode_rewards:
...
if __name__ == "__main__":
    ...
    # Prevents RuntimeError during cuda init
