...
class World(object):
    ...
    def restart(self):
        ...
        self.gnss_sensor = GnssSensor(self.player) # 即 美国国防部的 全球定位 GPS
        self.imu_sensor = IMUSensor(self.player) # 惯性测量单元
    ...
    def parse_events(self, client, world, clock, sync_mode):
        ...
        if not self._autopilot_enabled:
            if isinstance(self._control, carla.VehicleControl):
                ...
                # Apply control
                if not self._ackermann_enabled:
                    print("self._control.throttle = " + str(self._control.throttle) )
...
class HUD(object):
    ...
    def tick(self, world, clock):
        ...
        o = t.rotation.get_forward_vector()
        col_res = world.collision_sensor.get_collision(self.frame)
        lane_check = world.lane_invasion_sensor.lane_check(self.frame)

        if col_res != (0,0,0):
            self.collision_num = col_res
        # print( col_num )
        # print("ui frame = " + str(self.frame))
        # 添加额外要显示的信息collision_vehicle
        self._info_text_ex = [
            'location:(%5.1f,%5.1f,%5.1f)' % (t.location.x, t.location.y, t.location.z),
            'orientation:(%5.1f,%5.1f,%5.1f)' % (o.x, o.y, o.z),
            'rotation:(%5.1f,%5.1f,%5.1f)' % (t.rotation.yaw, t.rotation.pitch, t.rotation.roll),
            'Speed:   % 15.0f km/h' % speed,
            'collision:(%5.1f,%5.1f,%5.1f)' % col_res,
            'otherlane:   % 13.0f ' % lane_check[0],
            'offroad:   % 15.0f ' % lane_check[1],
            ]
    ...
    def render(self, display):
        ...
        self.render_ex(display)


    # 绘制额外的信息
    def render_ex(self, display): 
        # 定义一个220 宽的面板
        info_surface_ex = pygame.Surface((220, self.dim[1]))
        info_surface_ex.set_alpha(100)
        # 绘制在x = 220，y=0 的 位置上
        display.blit(info_surface_ex, (220, 0))

        # 默认偏移
        v_offset = 4
        for item in self._info_text_ex:
            surface_ex = self._font_mono.render(item, True, (255, 255, 255))
            display.blit(surface_ex, (8 + 220 , v_offset))
            # 每行文字高度
            v_offset += 18
