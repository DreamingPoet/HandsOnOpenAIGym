...
live_carla_processes = set()  # 保存所有已经启动的 carla server 的进程，方便关闭所有的进程
def cleanup():
...
MAP_NAMES = ["Town01", "Town02"]
# 默认环境配置
ENV_CONFIG = {
    "discrete_actions": True, # 是否采用离散的 action
    "use_image_only_observations": True,  # 从 observations 中排除 high-level planner 输入(inputs) 和 目标(goal) 信息
    "server_map": "/Game/Maps/" + MAP_NAMES[0], # 启动的地图名称
    "scenarios": None, # 动态场景信息
    "framestack": 2,  # 栈帧 ， 只支持 1 或 2 
    "enable_planner": True, # 
    "use_depth_camera": False,  # 使用深度相机
    "early_terminate_on_collision": True,
    "verbose": False, # 冗余
    "render" : True,  # 是否开启渲染模式
    "render_x_res": 800, # 渲染 分辨率
    "render_y_res": 600, # 渲染 分辨
    "x_res": 80,
    "y_res": 80,
    "seed": 1
}
...
class CarlaEnv(gym.Env):
    ...
    def __init__(self, config=ENV_CONFIG):
        ...
        """
        self.config = config
        self.city = self.config["server_map"].split("/")[-1] # 获取地图名称
    ...
    def init_server(self):
        ...
        self.server_process = 1 # carla 服务的进程
